// work-flow/src/components/DashboardSections/Chats/facebook/FacebookChat.jsx
import React, { useState, useEffect } from 'react';
import { Search, Send, Plus, Link, LogIn, MessageCircle, AlertCircle } from 'lucide-react';
...
const FacebookChat = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await socialMediaAPI.sendFacebookMessage({
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading Facebook accounts...</p>
        </div>
      </div>
    );
  }
  ...
  if (accounts.length === 0) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-8">
        <ConnectionStatus isConnected={false} platform="Facebook" />
        <div className="bg-blue-500 p-4 rounded-full mb-6">
          <MessageCircle className="h-12 w-12 text-white" />
        </div>
        <h3 className="text-xl font-semibold text-gray-900 mb-2">Connect Facebook Account</h3>
        <p className="text-gray-600 text-center mb-6 max-w-md">
          Connect your Facebook business page to start managing messages and conversations.
        </p>
        <button
          onClick={handleConnectFacebook}
          className="flex items-center gap-2 px-6 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-all shadow-md"
        >
          <LogIn className="h-5 w-5" />
          Connect with Facebook
        </button>
        <p className="mt-4 text-sm text-gray-500 text-center max-w-md">
          Note: You'll need to connect your Facebook business page with messaging permissions.
        </p>
      </div>
    );
  }
            ...
            .map((account) => (
              <div
                ...
                className={`p-4 border-b border-gray-100 cursor-pointer hover:bg-gray-50 ${
                  selectedAccount?._id === account._id ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''
                }`}
            ...
            <div className="bg-white border-b border-gray-200 p-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Facebook Business Page</p>
                </div>
              </div>
            </div>
              ...
              {messages[selectedAccount._id] && messages[selectedAccount._id].length > 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-gray-500">
                  ...
                  <p className="text-center max-w-md">
                    Start a conversation with your Facebook followers. Messages will appear here once received.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="bg-white border-t border-gray-200 p-4">
              <div className="flex items-center space-x-3">
                ...
                <button
                  ...
                  className="bg-blue-500 hover:bg-blue-600 disabled:bg-gray-300 text-white p-2 rounded-full transition-all"
                >
          ...
          <div className="h-full flex items-center justify-center">
            <div className="text-center p-8">
              ...
              <p className="text-gray-500">
                Choose a Facebook page from the list to start managing messages.
              </p>
            </div>
          </div>
        )}
